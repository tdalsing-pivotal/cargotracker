...
public class EventBackingBean implements Serializable {
    ...
    // used to check if the filled are ok
    private boolean inputsOk = false;
    ...
    public void init() {
        ...
        for (CargoRoute route : cargos) {
            if (route.isRouted() && !route.isClaimed()) {
                // we just need getRoutedUnclaimedCargos
    ...
    public Date getCompletionDate() {
        // todo : can a completion be in the past?
    ...
    public String handleEventSubmission() {
        ...
        HandlingEventRegistrationAttempt attempt = new HandlingEventRegistrationAttempt(registrationTime, completionDate, trackingId, selectedVoyage, type, unLocode);
